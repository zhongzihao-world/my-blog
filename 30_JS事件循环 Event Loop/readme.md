...
刚学前端的时候一直听别人说 JS 是单线程、单线程、单线程的，那么什么是线程？
...
打开 windows 任务管理器，可以看到浏览器开了很多个进程，每一个 tab 页都是单独的一个进程，一个页面崩溃以后并不会影响其他页面
...
为什么 js 引擎是单线程的，一个原因是多线程复杂度会更高，另一个问题是结果可能是不可预期的：假设 JS 引擎是多线程的，有一个 div，A 线程获取到该节点设置了属性，B 线程又删除了该节点，so what？该怎么操作呢？

或许这就是为什么 JS 引擎是单线程的，代码从上而下顺序的预期执行，虽然降低了编程成本，但也有其他问题，如果某个操作很耗时间，比如，某个计算操作 for 循环遍历 10000 万次，那么这个线程就卡住了......

GUI 渲染线程与 JS 引擎线程互斥的，是为了防止渲染出现不可预期的结果，因为 JS 是可以获取 dom 的，如果修改这些元素属性同时渲染界面（即 JS 线程和 UI 线程同时运行），那么渲染线程前后获得的元素数据就可能不一致了。所以 JS 线程执行的时候，渲染线程会被挂起；渲染线程执行的时候，JS 线程会挂起，所以 JS 会阻塞页面加载，这也是为什么 JS 代码要放在 header 之后，body 尾部之前的原因。
...
- 1. DNS 解析 URL
- 2. 浏览器与服务器建立连接（tcp 协议、三次握手）
- 3. 解析 html 成 dom 树、解析 css 成 cssobj
...
因为同步执行异步任务比较耗时间，而且代码中绝大部分都是同步代码，所以我们可以这些异步任务交给其他线程去执行，如定时触发器线程、异步 http 请求线程等。

那么异步任务同步任务怎么协调执行呢，就是通过 JS 事件循环，即调度任务的过程：

- 执行代码，如果是同步任务，就直接在主线程上执行，形成一个执行栈
- 当遇到异步任务的时候如网络请求等，就交给其他线程执行,当异步任务执行完了，就往事件队列里面塞一个回调函数
- 一旦执行栈中的所有同步任务执行完毕（即执行栈空），就会读取事件队列，取一个任务塞到执行栈中，开始执行
- 一直重复步骤 3

![](https://upload-images.jianshu.io/upload_images/10390288-58910e39b1867065.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
